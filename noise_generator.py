...
class LoopableNoiseGenerator(NoiseGenerator):
    ...
    def _init_synchronized_modulation_rates(self):
        ...
        self.sync_rate_low = max(1, int(0.05 * loop_length_seconds)) / loop_length_seconds
        self.sync_rate_mid = max(2, int(0.13 * loop_length_seconds)) / loop_length_seconds
        self.sync_rate_high = max(3, int(0.27 * loop_length_seconds)) / loop_length_seconds
        
        # For decorrelation modulation (very slow)
        self.sync_rate_decorr = max(1, int(0.033 * loop_length_seconds)) / loop_length_seconds
        
        # For Haas delay modulation (also very slow)
        self.sync_rate_haas = max(1, int(0.025 * loop_length_seconds)) / loop_length_seconds
        
        # For pitch variations (extremely slow)
        self.sync_rate_pitch_low = max(1, int(0.017 * loop_length_seconds)) / loop_length_seconds
        self.sync_rate_pitch_mid = max(1, int(0.022 * loop_length_seconds)) / loop_length_seconds
        self.sync_rate_pitch_high = max(1, int(0.029 * loop_length_seconds)) / loop_length_seconds
    ...
    def _generate_loopable_noise_spectrum(self, noise_type, block_size):
        ...
        left_magnitudes = spectral_filter.copy()
        right_magnitudes = spectral_filter.copy()
        
        # Apply the loopable phases to create complex spectrum
        left_phases = self.loop_phases.copy()
        
        # For right channel, apply decorrelation with frequency-dependent phase shift
        # For brown noise, use less decorrelation in low frequencies for better spatial coherence
        if noise_type == 'brown':
            # Create frequency-dependent decorrelation that preserves low-end coherence
            phase_shift = cp.zeros_like(self.decorrelation_phases)
            # Less decorrelation below 150Hz
            phase_shift[:self.low_bin] = self.decorrelation_phases[:self.low_bin] * 0.5
            # Normal decorrelation above 150Hz
            phase_shift[self.low_bin:] = self.decorrelation_phases[self.low_bin:]
            right_phases = (left_phases + cp.angle(phase_shift)) % (2*cp.pi)
        else:
            right_phases = (left_phases + cp.angle(self.decorrelation_phases)) % (2*cp.pi)
    ...
    def _apply_natural_modulation_loopable(self, noise_block, block_start_idx, block_size):
        ...
        t = cp.linspace(loop_position, loop_position + block_size/self.loop_length_samples, 
                      block_size, endpoint=False) % 1.0
        ...
        low_mod = 1.0 + 0.03 * cp.sin(2 * cp.pi * t * self.sync_rate_low * self.config.loop_length)
        mid_mod = 1.0 + 0.025 * cp.sin(2 * cp.pi * t * self.sync_rate_mid * self.config.loop_length)
        high_mod = 1.0 + 0.02 * cp.sin(2 * cp.pi * t * self.sync_rate_high * self.config.loop_length)
        ...
        # Get frequency bin indices
        n_bins = len(left_fft)
        low_idx = self.low_bin
        mid_idx = self.mid_bin
    ...
    def _apply_dynamic_stereo_parameters_loopable(self, noise_block, block_start_idx, block_size):
        ...
        if self.config.enhanced_stereo:
            ...
            # Calculate frequency band boundaries
            low_freq_idx = self.low_bin
            mid_freq_idx = self.mid_bin
        ...
        if self.config.haas_effect:
            ...
            transition = cp.zeros(n_freqs, dtype=self.precision)
            transition[:cutoff_bin-xfade_width] = 0.0
            transition[cutoff_bin+xfade_width:] = 0.7
    ...
    def _apply_micro_pitch_variations_loopable(self, noise_block, block_start_idx, block_size):
        """Apply subtle micro-pitch variations with loop-synchronized modulation"""
        # Only apply if natural modulation is enabled
        if not self.config.natural_modulation:
            return noise_block
        ...
        for ch in range(2):
            ...
            # Calculate bin shifts with synchronized modulation
            if len(low_bins) > 0:
                low_bin_shifts = low_bins * low_pitch_mod
                low_bin_ints = cp.floor(low_bins + low_bin_shifts).astype(cp.int32)
                low_bin_fracs = (low_bins + low_bin_shifts) - low_bin_ints
    ...
    def _apply_lfo_modulation_loopable(self, noise_block, block_start_idx, block_size):
        ...
        modulation_depth = 10**(1.0/20) - 10**(-1.0/20)  # ±1dB in linear scale
        modulation = 1.0 + modulation_depth/2 * cp.sin(2 * cp.pi * self.sync_lfo_rate * t * self.config.loop_length)
    ...
    def generate_block(self, block_size, block_start_idx=0):
        ...
        if white_gain > 0.001:
            ...
            frequency_domain_buffers['white'] = (left_white, right_white)
        
        if pink_gain > 0.001:
            left_pink, right_pink = self._generate_loopable_noise_spectrum('pink', block_size)
            frequency_domain_buffers['pink'] = (left_pink, right_pink)
        
        if brown_gain > 0.001:
            left_brown, right_brown = self._generate_loopable_noise_spectrum('brown', block_size)
            frequency_domain_buffers['brown'] = (left_brown, right_brown)
        
        # Mix the noise types in frequency domain with proper gains
        left_spectrum = cp.zeros(block_size//2 + 1, dtype=cp.complex128)
        right_spectrum = cp.zeros(block_size//2 + 1, dtype=cp.complex128)
